...
This archived setup is superseded by the canonical production guide at `../docs/PRO.md`.
...
If Redis is unreachable at startup or during a request, `GuardPro` throws `SHARED_BUDGET_UNAVAILABLE` by default. This prevents a shared budget from silently becoming independent process-local budgets. `allowLocalFallback: true` explicitly opts into best-effort local tracking, which is not shared enforcement.
