...
describe('CLI - Strict Behavioral Tests', () => {
  ...
  describe('check command', () => {
    test(
      'should output exact safe request format',
      async () => {
        const { stdout, stderr } = await execAsync('node dist/cli/index.js check "hello world" --model gpt-4', {
          timeout: 10000,
        });
        ...
        // Header line
        expect(lines[0]).toMatch(/^\ud83d\udee1\ufe0f {2}AI EXECUTION FIREWALL$/);
        ...
        // Status line
        expect(lines[3]).toMatch(/^\u2705 SAFE TO PROCEED$/);

        // Risk Level line (per spec #13 - must expose risk)
        expect(lines[4]).toMatch(/^RISK: (LOW|MEDIUM|HIGH|CRITICAL)$/);

        // Danger score line
        expect(lines[5]).toMatch(/^DECISION: ALLOW$/);

        // Cost line
        expect(lines[6]).toMatch(/^COST: \$[0-9.]+$/);

        // Universal output format (per spec #11)
        expect(stdout).toContain('Universal Format:');
    ...
    test(
      'should output exact duplicate detection format',
      async () => {
        // First request - safe
    ...
    test(
      'should output exact loop/kill-switch format',
      async () => {
    ...
    test(
      'should output exact cost spike format',
      async () => {
  ...
  describe('report command', () => {
    test(
      'should output exact report format',
      async () => {
    ...
    test(
      'should report exact count of 3 requests',
      async () => {
  ...
  describe('blocked command', () => {
    test(
      'should output exact blocked log format',
      async () => {
        ...
        // Block entry format is hash-only by default, so plaintext prompt is not shown.
        const blockLine = lines.find((l) => l.includes('[hash:'));
        expect(blockLine).toMatch(/^\[.+\] \[(duplicate|loop)\] \[hash:[a-f0-9]{64}\] \(Score: \d+\)$/);
      },
      TEST_TIMEOUT
    );

    test(
      'should show empty log when no blocks',
      async () => {
        detectionEngine.clear();
  ...
  describe('config command', () => {
    test(
      'should output exact config format',
      async () => {
        const { stdout } = await execAsync('node dist/cli/index.js config');

        const lines = stdout.split('\n').filter((l) => l.trim());

        expect(lines[0]).toMatch(/^\u2699\ufe0f {2}FIREWALL CONFIGURATION$/);
        expect(lines[1]).toMatch(/^={25,}$/);
  ...
  describe('CLI exit codes', () => {
    test(
      'should exit with code 0 on safe check',
      async () => {
    ...
    test(
      'should exit with code 0 on report command',
      async () => {
    ...
    test(
      'should exit with code 1 on invalid command',
      async () => {
